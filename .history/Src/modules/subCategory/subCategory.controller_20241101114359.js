...
export const create = async (req, res) => {
  ...
  const { secure_url, public_id } = await uploadImage(req.file.path, 'ecommerce1/subCategories');
  req.body.image = { secure_url, public_id };
  req.body.createdBy = req.user._id;
  req.body.updatedBy = req.user._id;

  // إنشاء الفئة في قاعدة البيانات
  const subcCategory = await categoryModel.create(req.body);
  return res.json({ message: "success", subcCategory });
};
